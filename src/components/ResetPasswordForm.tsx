...
export function ResetPasswordForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!resetToken) {
      setFormError("This reset link is invalid or missing. Please request a new one.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setFormError(body?.error ?? "Reset failed. Please try again.");
        setSubmitting(false);
        return;
      }
        ...
        <div className="mt-6">
          ...
          <div className="relative mt-1.5">
            <input
              ...
              className={`w-full rounded-md border px-3 py-2 pr-10 text-[13px] text-[var(--ink)] focus:outline-none focus:ring-2 disabled:cursor-not-allowed disabled:opacity-60 ${
      ...
      {resetSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div
            ...
            className="w-full max-w-[380px] rounded-lg border border-[var(--border)] bg-[var(--surface)] p-6 text-center shadow-lg"
          >
            <h2
            ...
            <Link
              href="/signin"
              className="mt-5 inline-block w-full rounded-md bg-[var(--accent)] px-3.5 py-2 text-[13px] font-medium text-white shadow-sm hover:bg-[var(--accent-strong)]"
            >
              Back to Sign In
            </Link>
          </div>
